...
plist_lin        <- readRDS("./Output/gtex_fits_lin/plist.Rds")
pi0mat_lin       <- readRDS("./Output/gtex_fits_lin/pi0mat.Rds")
betahat_list_lin <- readRDS("./Output/gtex_fits_lin/betahat_list.Rds")

## Give pi0mat the fits when using the ctl genes from Lin et al.
which_swap           <- match(colnames(pi0mat_lin), colnames(pi0mat))
pi0mat[, which_swap] <- pi0mat_lin
...
for (tissue_index in 1:length(tissue_vec)) {
    ...
    ## Give pmat the fits when using the ctl genes from Lin et al.
    pmat               <- plist[[tissue_index]]
    pmat_lin           <- plist_lin[[tissue_index]]
    which_swap         <- match(colnames(pmat_lin), colnames(pmat))
    pmat[, which_swap] <- pmat_lin
    ...
    ## Give betamat the fits when using the ctl genes from Lin et al.
    betamat               <- betahat_list[[tissue_index]]
    betamat_lin           <- betahat_list_lin[[tissue_index]]
    which_swap            <- match(colnames(betamat_lin), colnames(betamat))
    betamat[, which_swap] <- betamat_lin
    ...
    cat(good_tissue_labels[tissue_index], ":", tissue_index, "\n")
}
...
for (tissue_index in 1:length(tissue_vec)) {
    ...
    pmat <- as_data_frame(pmat) %>% select(contains("ash_"), contains("qvalue_"))
